...
class MainActivity : AppCompatActivity() {
    ...
    fun changeViewEnable() = when (connectionMode) {
        ...
        "aidl" -> {
            ...
            binding.aidProgress.progress = 0
        }
    }
    ...
    val aidlConnection: ServiceConnection = object: ServiceConnection {
        override fun onServiceConnected(p0: ComponentName?, p1: IBinder?) {
            ...
            aidlService!!.start()
            binding.aidProgress.max = aidlService!!.maxDuration
            val backgroundScope = CoroutineScope(Dispatchers.Default + Job())
            aidlJob = backgroundScope.launch {
                while(binding.aidProgress.progress < binding.aidProgress.max){
                    delay(1000)
                    binding.aidProgress.incrementProgressBy(1000)
                }
            }
            connectionMode = "aidl"
            changeViewEnable()
